...
// Ensure the correct path or create the type if it doesn't exist
...
describe('LoginService', () => {
  ...
  it('deve exibir erro ao falhar no login', () => {
    ...
    service.login('test@example.com', 'wrong-password').subscribe({
      error: () => {
        expect(toastrSpy.error).toHaveBeenCalledWith('Login failed! Please check your credentials.');
      },
    });
  ...
  it('deve buscar userId se não existir e armazená-lo na sessão', () => {
    const mockResponse: LoginResponse = {
      id: undefined,
      token: 'test-token',
      name: 'John Doe',
    ...
    const mockUsers = [{ id: '3', name: 'John Doe' }];

    spyOn(service, 'getUserIdByName').and.returnValue(
      service.getAllUsers().pipe(map((users) => users[0].id))
    );
    ...
    expect(sessionStorage.getItem('userId')).toBe('3');
